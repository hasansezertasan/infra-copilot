...
Ten of the 33 have no such header — `.claude-plugin/marketplace.json`,
...
`checks/hcp-apply-scope.sh`, `checks/leaf-cloud.sh`, and
`checks/hcp-current-plan.sh` — so a missing header proves nothing.

`.ai-rulez-generated.json` is the authoritative list — 33 paths today. If a file is in
