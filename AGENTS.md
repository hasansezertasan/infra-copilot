...
is listed (39 are), edit its source under `.ai-rulez/` instead, then run `make generate`.

Most generated files carry an `AI-RULEZ :: GENERATED FILE — DO NOT EDIT` header, but
**13 of the 39 do not** — every non-Markdown output, because a Markdown comment is not
