...
| `vcs-connect`'s check reads `/organizations/<org>/oauth-clients`, which is organization-scoped, so it reads red once the plan-only credential is in place. | Expected, not a regression: phase 1 is complete by then and is the phase that legitimately needs the user token. `status` reports it as `–` with that reason. **Do not** grant the team "Manage version control settings" to turn it green — that is an organization permission, and organization permissions are the self-elevation risk above. |
